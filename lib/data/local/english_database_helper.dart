...
class EnglishDatabaseHelper {
  ...
  Future openDb() async {
    ...
    db = await openDatabase(path.join(dbPath, _dbName), version: 1,
        onCreate: (_db, version) async {
      _db.execute(
          'CREATE TABLE $topicTableName (id INTEGER PRIMARY KEY, name TEXT, description_en TEXT, image_url TEXT)');
      _db.execute(
